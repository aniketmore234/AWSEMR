...
mysql < grant all privileges on *.* to 'root'@'$1' IDENTIFIED BY 'password' WITH GRANT OPTION

mysql < grant all privileges on *.* to 'root'@'$2' IDENTIFIED BY 'password' WITH GRANT OPTION

mysql < grant all privileges on *.* to 'root'@'$3' IDENTIFIED BY 'password' WITH GRANT OPTION
...
sqoop import --connect jdbc:mysql://"$1":3306/sms --username root --password password --split-by date_id_id --table dateDim --target-dir hdfs:///user/hive/warehouse/stockdata --fields-terminated-by "," --hive-import --create-hive-table --hive-table stockdata.dateDim
